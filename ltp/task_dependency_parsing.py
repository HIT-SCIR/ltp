...
from ltp.metrics.metric import Tree, CharTree
...
def metric_builder(dataset):
    for idx, name in enumerate(dataset[datasets.Split.TRAIN].features['labels'].feature.names):
...
def validation_method(metric: Tree, task=f'dep', preffix='val', ret=False):
