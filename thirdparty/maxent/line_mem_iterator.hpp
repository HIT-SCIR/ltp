...
#ifndef line_mem_iterator_H
#define line_mem_iterator_H
    ...
    : public iterator<input_iterator_tag, _Tp, _Dist, const _Tp*, const _Tp&>
{
    ...
    public:      
        line_mem_iterator() : _M_data_begin(0), _M_data_end(0), _M_ok(false) {}

        line_mem_iterator(const char* begin, const char* end) :
            _M_data_begin(begin), _M_data_end(end) { 
            _M_value.first = begin;
            _M_value.second = begin - 1;
            _M_read();
        }

        // compiler generated copy constructor is ok
        // line_mem_iterator(const line_mem_iterator& __obj) 

        const _Tp& operator*() const { return _M_value; }

        const _Tp* operator->() const { return &(operator*()); }

        line_mem_iterator& operator++() 
            { _M_read(); return *this; }

        line_mem_iterator operator++(int)  
            {
                line_mem_iterator __tmp = *this;
                _M_read();
                return __tmp;
            }

        bool _M_equal(const line_mem_iterator& __x) const
            { return (_M_ok == __x._M_ok) && 
                (!_M_ok || (_M_data_begin == __x._M_data_begin &&
                            _M_data_end == __x._M_data_end));}

    private:      
        void _M_read()  // advance one line
            {
                _M_ok = (_M_value.first < _M_data_end) ? true : false;
                if (_M_ok) 
                {
                    _M_value.first = _M_value.second + 1;
                    const char* p = _M_value.second + 1;
                    while (p < _M_data_end && *p != '\n')
                        ++p;
                    _M_value.second = p;
                    _M_ok = (_M_value.first < _M_data_end) ? true : false;
                }
...
inline bool 
operator==(const line_mem_iterator<_Tp, _CharT, _Traits, _Dist>& __x,
        const line_mem_iterator<_Tp, _CharT, _Traits, _Dist>& __y) 
