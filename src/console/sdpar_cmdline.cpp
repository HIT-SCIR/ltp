...
#define EXECUTABLE "sdpar_cmdline"
...
void multithreaded_parse( void * args) {
  ...
  while (true) {
    ...
    heads.clear();
    deprels.clear();
    sdparser_parse(model, words, postags, heads, deprels);
...
int main(int argc, char * argv[]) {
  ...
  std::string parser_model = "ltp_data/sdparser.model";
  if (vm.count("parser-model")) {
    parser_model= vm["parser-model"].as<std::string>();
  }

  void *engine = sdparser_create_parser(parser_model.c_str());
  if (!engine) {
    return 1;
  }
  ...
  delete dispatcher;
  sdparser_release_parser(engine);
  return 0;
}
